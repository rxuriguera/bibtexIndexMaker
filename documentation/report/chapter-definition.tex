...
\paragraph{}
La solució que hem trobat per poder tirar endavant consisteix en fer ús de les biblioteques digitals descrites a la introducció. La majoria d'elles estan indexades pels principals cercadors disponibles a Internet. El diagrama actualitzat es mostra a la figura \ref{fig:definition:extraction-diagram}. Bàsicament, per cada fitxer l'aplicació:
\begin{enumerate}
...
\paragraph{}
Qualsevol interessat pot agafar el codi i modificar-lo tan per afegir noves funcionalitats com per arreglar errors. La forma més fàcil de fer-ho és anar al repositori on es troba el projecte (\cite{repo}) i fer un \textit{fork}. D'aquesta manera, el nou codi sempre es podrà fusionar a la branca \textit{oficial}.
