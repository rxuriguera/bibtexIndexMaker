...
Acabem de veure com ho fa la nostra aplicació per cadascun dels problemes a solucionar per aconseguir l'objectiu d'extreure referències. En aquesta secció es mostren els resultats obtinguts una vegada hem unit les tres parts anteriors. Per realitzar les proves s'ha creat un conjunt aleatori de fitxers PDF corresponents a articles relacionats amb el camp de la informàtica. Això afecta força a les biblioteques per les quals es trobaran resultats, ja que molts articles d'aquest tipus acostumen a estar indexats per \textit{ACM Portal}, \textit{SpringerLink} i \textit{CiteSeerX}, per les quals sabem extreure'n les dades.

\paragraph{}
Els tres gràfics següents exposen els resultat depenent de l'estat de l'aplicació: el primer, quan no hi ha cap \textit{field wrapper} definit, només \textit{reference wrappers} (\ref{refextraction:reference-wrappers}); el segon, una vegada hem generat \textit{field wrappers} per les pàgines que més s'utilitzen; i el tercer, després de corregir algunes de les regles manualment.
...
\paragraph{}
El temps necessari per executar pot variar segons la connexió a Internet de la qual es disposa, tot i que en situacions \textit{normals} el temps mitjà per fitxer sol estar entre 1 i 1.5 segons. Una de les conseqüències de fer servir diferents fils d'execució és que si l'espera d'algun dels fitxers tarda més del compte, els altres es poden anar processant.
