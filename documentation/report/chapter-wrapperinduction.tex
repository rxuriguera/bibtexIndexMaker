...
\paragraph{}
Per generar les expressions regulars, agafem el text dels elements HTML que ens han proporcionat les \textit{path rules} i fem una substitució del valor que volem obtenir per \texttt{(.*)}.

\paragraph{}
La tècnica que fem servir per decidir si hem de fusionar dues expressions o no és molt senzilla. Simplement fem \textit{string matching} de les expressions i mirem si tenen una similaritat major que un llindar establert. En cas afirmatiu, obtenim els blocs no coincidents de les dues cadenes de caràcters i els substituïm per \texttt{(?:.*)} \footnote{En \textit{Python}, la fòrmula \texttt{(?:...)} indica que es pot descartar el que hi ha dins els parèntesis.}. 
